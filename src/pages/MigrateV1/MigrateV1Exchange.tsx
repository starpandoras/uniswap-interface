...
import { DEFAULT_DEADLINE_FROM_NOW, INITIAL_ALLOWED_SLIPPAGE, WETH_EXTENDED } from '../../constants'
    ...
    <>
      <AutoRow style={{ justifyContent: 'flex-start', width: 'fit-content' }}>
        ...
        <div style={{ marginLeft: '.75rem' }}>
          <TYPE.mediumHeader>
            ...
            {chainId && token.equals(WETH_EXTENDED[chainId]) ? 'WETH' : token.symbol}/ETH
          </TYPE.mediumHeader>
        </div>
      </AutoRow>
      ...
      <RowBetween my="1rem">
        <Text fontSize={16} fontWeight={500}>
          Pooled {chainId && token.equals(WETH_EXTENDED[chainId]) ? 'WETH' : token.symbol}:
        </Text>
        <RowFixed>
...
function V1PairMigration({ liquidityTokenAmount, token }: { liquidityTokenAmount: TokenAmount; token: Token }) {
  ...
  const [v2PairState, v2Pair] = usePair(chainId ? WETH_EXTENDED[chainId] : undefined, token)
  const isFirstLiquidityProvider: boolean = v2PairState === PairState.NOT_EXISTS

  const v2SpotPrice = chainId && v2Pair ? v2Pair.reserveOf(token).divide(v2Pair.reserveOf(WETH_EXTENDED[chainId])) : undefined
  ...
  return (
    <BodyWrapper style={{ padding: 24 }}>
      <AutoColumn gap="16px">
        ...
        ) : validatedAddress && chainId && token?.equals(WETH_EXTENDED[chainId]) ? (
          <>
            <TYPE.body my={9} style={{ fontWeight: 400 }}>
